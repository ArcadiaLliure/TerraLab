...
def main():
    ...
    tiff_path = (
        r"e:\Desarrollo\TerraLab\TerraLab\data\light_pollution\C_DVNL 2022.tif"
    )
    ...
    print(
        f"Baking with Observer at {x_utm:.1f}, {y_utm:.1f}, elevation {ground_h:.1f}m..."
    )
    ...
    with open(
        r"e:\Desarrollo\TerraLab\TerraLab\clean_peaks.txt",
        "w",
        encoding="utf-8",
    ) as f:
        ...
        if pending_domes:
            ...
            sorted_by_intensity = sorted(
                pending_domes,
                key=lambda x: light_domes[x["idx"]],
                reverse=True,
            )
            used_indices = set()
        ...
        for p in pending_domes:
            ...
            dist = p["dist"]
            f.write(
                f"Azimuth: {az:.1f}, Radiance Sum: {rad_val:.2f}, Peak Dist: {dist:.0f}m\n"
            )
        ...
        for az in [0, 90, 180, 270]:
            ...
            f.write(
                f"Horizon Angle at {az} deg: {np.degrees(max_angles[idx]):.2f} deg\n"
            )
