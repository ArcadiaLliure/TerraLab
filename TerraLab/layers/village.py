...
# --- CONFIGURATION ---
VILLAGE_SCALE = (
    0.6  # Adjust this to resize all village objects (houses, trees, lanterns)
)
...
class VillageTheme:
    def __init__(self):
        ...
        self.night_mtn_2 = QColor(
            18, 22, 35
        )  # Faint mist/blue for far (almost sky color)
        ...
        # Nature
        self.water_day = QColor(
            130, 200, 240, 200
        )  # Semi-transparent Pastel Blue
    ...
    def get_color(self, name, t_night):
        """Obte color de la instancia de VillageTheme.

        Par?metres:
        - name (Any): Valor del parametre 'name'.
        - t_night (Any): Valor del parametre 't_night'.
        ...
        if name == "ground":
            return lerp_c(self.day_ground, self.night_ground, t_night)
        if name == "mtn_near":
            return lerp_c(self.day_mtn_1, self.night_mtn_1, t_night)
        if name == "mtn_far":
            return lerp_c(self.day_mtn_2, self.night_mtn_2, t_night)

        if name == "wall":
            return lerp_c(self.day_house_wall, self.night_house_wall, t_night)
        if name == "wood":
            return lerp_c(self.day_house_wood, self.night_house_wood, t_night)
        if name == "roof":
            return lerp_c(self.day_house_roof, self.night_house_roof, t_night)

        if name == "water":
            return lerp_c(self.water_day, self.water_night, t_night)
        if name == "tree":
            return lerp_c(self.tree_day, self.tree_night, t_night)
        if name == "line":
            return lerp_c(self.line_day, self.line_night, t_night)
...
class MinkaHouse:
    ...
    def draw(self, painter, projection_fn, theme, t_night, view_az, fov):
        # 1. Scale Calculation
        """Executa el metode draw de la classe MinkaHouse.

        Par?metres:
        - painter (Any): Valor del parametre 'painter'.
        - projection_fn (Any): Valor del parametre 'projection_fn'.
        - theme (Any): Valor del parametre 'theme'.
        - t_night (Any): Valor del parametre 't_night'.
        - view_az (Any): Valor del parametre 'view_az'.
        - fov (Any): Valor del parametre 'fov'.
        ...
        def draw_cube(cx, cz, bw, bh, bd, is_stone=False):
            ...
            sv = [
                verts.get((side_x, 0, -1)),
                verts.get((side_x, 0, 1)),
                verts.get((side_x, 1, 1)),
                verts.get((side_x, 1, -1)),
            ]
            ...
            if not is_stone:
                ...
                if l_top and l_bot:
                    painter.drawLine(QPointF(*l_top), QPointF(*l_bot))

                # Right edge
                r_top = verts.get((1, 1, 1))
                r_bot = verts.get((1, 0, 1))
                if r_top and r_bot:
                    painter.drawLine(QPointF(*r_top), QPointF(*r_bot))

                # Top horizontal beam
                if l_top and r_top:
                    painter.drawLine(QPointF(*l_top), QPointF(*r_top))

            if is_stone:
                return  # No windows on stone base
...
class VillageLantern:
    ...
    def __init__(self, rng, az_range, alt_range):
        ...
        self.style = rng.choice(
            ["ikekomi", "tachi", "yukimi"]
        )  # ground, standing, snow-viewing

    def draw(self, painter, projection_fn, theme, t_night, view_az, fov):
        # Visibility check
        """Executa el metode draw de la classe VillageLantern.
        ...
        painter.drawRect(
            QRectF(base.x() - base_w / 2, base.y() - base_h, base_w, base_h)
        )
        ...
        painter.drawRect(QRectF(base.x() - col_w / 2, col_top, col_w, col_h))
        ...
        painter.setBrush(c_stone_dark)
        painter.drawRect(
            QRectF(base.x() - chudai_w / 2, chudai_top, chudai_w, chudai_h)
        )
        ...
        painter.drawRect(
            QRectF(
                base.x() - hibukuro_w / 2, hibukuro_top, hibukuro_w, hibukuro_h
            )
        )
        ...
        painter.drawEllipse(
            QPointF(base.x(), roof_top - hoju_r), hoju_r, hoju_r
        )
...
class VillageTree:
    ...
    def draw(self, painter, projection_fn, theme, t_night, view_az, fov):
        # Projection check
        """Executa el metode draw de la classe VillageTree.
        ...
        if self.is_sakura:
            # Pink/White
            c_leaf_base = (
                QColor(255, 192, 203) if t_night < 0.5 else QColor(100, 60, 70)
            )
            # Consistent variation based on seed (no flickering)
            if rng.random() > 0.5:
                c_leaf_base = c_leaf_base.lighter(110)
        else:
...
class VillageOverlay(QObject):
    ...
    def draw_with_projection(
        ...
        # FOV Calc for Scale
        """Executa el metode draw_with_projection de la classe VillageOverlay.

        Par?metres:
        - painter (Any): Valor del parametre 'painter'.
        - projection_fn (Any): Valor del parametre 'projection_fn'.
        - width (Any): Valor del parametre 'width'.
        - height (Any): Valor del parametre 'height'.
        - current_hour (Any): Valor del parametre 'current_hour'.
        - current_azimuth (Any): Valor del parametre 'current_azimuth'.
        - zoom_level (Any): Valor del parametre 'zoom_level'.
        - elevation_angle (Any): Valor del parametre 'elevation_angle'.
        ...
        if elevation_angle < 75.0:
            ...
            all_objects.sort(
                key=lambda x: getattr(x[1], "custom_scale", 1.0)
            )  # Smallest (Far) first
    ...
    def handle_click_projection(self, click_az, click_alt, view_az, fov):
        """Executa el metode handle_click_projection de la classe VillageOverlay.

        Par?metres:
        - click_az (Any): Valor del parametre 'click_az'.
        - click_alt (Any): Valor del parametre 'click_alt'.
        - view_az (Any): Valor del parametre 'view_az'.
        - fov (Any): Valor del parametre 'fov'.
