"""Runtime helpers for the canonical astronomical canvas."""
    ...
    if isinstance(moon, dict):
        ...
        try:
            ...
            if illumination is None:
                ...
                illumination = (1.0 - math.cos(math.radians(separation))) / 2.0
...
def canvas_update_skyfield_cache(canvas, ut_hour, day_of_year):
    ...
    if cache_valid:
        return
    # Regenerate Cache
    if not hasattr(self.parent_widget, "eph"):
        self._sf_cache = {
            "time": float(ut_hour),
    ...
    try:
        ...
        for key, (name, col, sz) in planet_defs.items():
            try:
                ...
                log_suppressed_exception(__name__, "canvas_update_skyfield_cache")
        # Eclipse Factor
        ...
        cache_data = {
            ...
            "moon": {
                "alt": alt_m_real.degrees,
                "az": az_m_real.degrees,
                "dist_km": d_moon_km,
                "rad_deg": moon_rad_deg,
                "sep_real": sep_real,
...
def canvas_scope_hud_star_count(canvas):
    ...
    try:
        ...
        boundary_screen = ctrl._project_valid(
            boundary_sky, self.project_universal_stereo
        )
    except Exception:
        return self._visible_star_count_raw()
    ...
    try:
        if (
            np is not None
            and hasattr(self, "visible_stars_sx")
            and hasattr(self, "visible_stars_sy")
        ):
            ...
            if sx_arr.size > 0 and sy_arr.size > 0:
                ...
                for i in idx:
                    if hole.contains(
                        QPointF(float(sx_arr[i]), float(sy_arr[i]))
                    ):
                        count += 1
                return int(count)
    except Exception:
        log_suppressed_exception(__name__, "canvas_scope_hud_star_count")
...
def canvas_set_selected_target(canvas, target):
    ...
    if isinstance(target, dict) and target.get("kind") == "star":
        ...
        display_name = getattr(obj, "common_name", None) or getattr(
            obj, "name", "NGC"
        )
...
def canvas_scope_hud_extra_lines(canvas, ut_hour: float, day_of_year_utc: int):
    ...
    line = getTraduction("Scope.HudCoords", "RA {ra} | Dec {dec}").format(
    ...
    metrics = getattr(self.parent_widget, "scope_atmo_metrics", {}).get(
        "hud_metrics", {}
    )
    if isinstance(metrics, dict):
        ...
        if exit_p is not None:
            lines.append(
                getTraduction(
                    "Scope.HudExitPupil", "Exit pupil: {v:.2f} mm"
                ).format(v=float(exit_p))
            )
        if x_air is not None:
            lines.append(
                getTraduction("Scope.HudAirmass", "Airmass X: {v:.3f}").format(
                    v=float(x_air)
                )
            )
        if k_ext is not None:
            lines.append(
                getTraduction(
                    "Scope.HudExtinctionK", "Extinction k: {v:.3f}"
                ).format(v=float(k_ext))
            )
        if loss_mag is not None:
            lines.append(
                getTraduction("Scope.HudLossMag", "Loss: {v:.3f} mag").format(
                    v=float(loss_mag)
                )
            )
        if trans is not None:
            lines.append(
                getTraduction(
                    "Scope.HudTransmission", "Transmission: {v:.3f}"
                ).format(v=float(trans))
            )
    scope_data_state = str(
        getattr(self.parent_widget, "_scope_data_state", "ready_deep")
        or "ready_deep"
    )
...
def canvas_log_positions(canvas):
    ...
    try:
        ...
        # Topo
        s_ra_topo, s_dec_topo, lst = self.get_topocentric_position(
            sun_ra, sun_dec, 149597870.7, lat, lon, jd_utc
        )
        m_ra_topo, m_dec_topo, _ = self.get_topocentric_position(
            moon_ra, moon_dec, m_dist, lat, lon, jd_utc
        )
        s_alt, s_az = self.sun_alt_az_from_ra_dec(
            s_ra_topo, s_dec_topo, lat, lst
        )
        m_alt, m_az = self.sun_alt_az_from_ra_dec(
            m_ra_topo, m_dec_topo, lat, lst
        )
        # Projected
        ...
        m_px = (
            f"{pt_moon[0]:.1f}, {pt_moon[1]:.1f}" if pt_moon else "OFF_SCREEN"
        )
        log_line = (
            f"[{timestamp}] Local={local_hour:.2f}h UT={ut_hour:.2f}h | "
            f"SUN: Alt={s_alt:.2f} Az={s_az:.2f} Px={s_px} | "
            f"MOON: Alt={m_alt:.2f} Az={m_az:.2f} Px={m_px}"
        )
        with open(
            os.path.join(log_dir, "pos_log_ctrl_l.txt"), "a", encoding="utf-8"
        ) as f:
            f.write(log_line + "\n")
        # Feedback
...
def canvas_paintEvent(canvas, event):
    ...
    try:
        ...
        scene_stage = getattr(
            self.parent_widget, "scene_load_stage", "scene_ready"
        )
        ...
        rise_loc, set_loc = self.calculate_sun_times(
            self.parent_widget.latitude, day_of_year
        )
        # Calculate Antipodal Sun Times (-Lat, +12h)
        rise_anti, set_anti = self.calculate_sun_times(
            -self.parent_widget.latitude, day_of_year
        )
        (solar_hour + 12.0) % 24.0
        ...
        if self.parent_widget is not None:
            ...
            eff_sun_alt, eff_sun_az = self.get_sun_alt_az(
                eff_hour, eff_lat, day_for_astro
            )
            ...
            if hasattr(self.parent_widget, "eph"):
                # Check if simulated time has changed significantly (manual time change)
                last_cached_hour = self._sf_cache.get("ut_hour", -999)
                time_changed = (
                    abs(ut_hour - last_cached_hour) > 0.001
                )  # ~3.6 seconds in simulation
                # Force update if time changed OR if enough real time has passed
                should_update = time_changed or (
                    current_time_ms - self._last_skyfield_update
                    > skyfield_update_interval
                )
                if should_update:
        ...
        if (
            ...
            self.draw_light_domes(
                painter,
                self.horizon_overlay.profile,
                eff_sun_alt,
                eclipse_dimming,
            )
        # === WEATHER SYSTEM ===
        ...
        if (not fast_interaction) or (now_weather - last_weather >= 0.20):
            self.weather.update_weather(
                day_of_year_utc, ut_hour, year=dt_utc.year
            )
        ...
        view_mag_limit = float(
            render_state.get("render_mag_limit", vm_state.scope_limit_mag)
        )
        ...
        if (
            ...
            and hasattr(self, "_sf_cache")
            and self._sf_cache.get("data")
        ):
        ...
        show_stars_layer = self._parent_checkbox_checked(
            "chk_enable_sky", default=True
        )
        show_milkyway_layer = self._parent_checkbox_checked(
            "chk_enable_milkyway", default=True
        )
        if scene_stage_rank >= 1 and (show_stars_layer or show_milkyway_layer):
            self.draw_stars(
                ...
                moon_mask=moon_mask,
                mag_limit=final_mag_limit,
                eff_lat=eff_lat,
                day_of_year=day_for_astro,
            )
        self.visible_sky_objects = []
        if hasattr(self.parent_widget, "eph"):
            self.draw_skyfield_objects(
                painter,
                ut_hour,
                day_for_astro,
        ...
        if not (self.scope_mode_enabled() and scope_motion):
            self.constellation_controller.draw(
                ...
                lambda ra, dec: self._ra_dec_to_alt_az(
        ...
        scope_loading_heavy = (
            ...
            or bool(
                getattr(self.parent_widget, "_scope_catalog_loading", False)
            )
            or bool(
                getattr(
                    self.parent_widget, "_scope_preload_in_progress", False
                )
            )
        )
        ngc_heavy_load = bool(
            self.scope_mode_enabled() and (scope_motion or scope_loading_heavy)
        )
        ...
        if hasattr(self.parent_widget, "chk_enable_horizon"):
            show_horizon = self._parent_checkbox_checked(
                "chk_enable_horizon", default=True
            )
        ...
        if show_horizon:
            ...
            if (
                ...
                if eff_sun_alt >= 0:
                    ...
                    tw_factor = (0 - eff_sun_alt) / 18.0
                tw_factor *= eclipse_dimming
                if tw_factor > 0.01:

                    def dome_callback(p, idx, dist):
                        return self._draw_single_city_dome(
                            p,
                            self.horizon_overlay.profile,
                            idx,
                            dist,
            ...
            if hasattr(self, "_dome_count") and self._dome_count > 0:
                ...
                if (
                    ...
                    and current_time - getattr(self, "_last_dome_log_time", 0)
                    > 2.0
                ):
                    print(
                        f"[AstroCanvas] City Domes Draw Call: {self._dome_count} centers found."
                    )
            ...
            display_focal = int(
                round(
                    max(
                        ...
                        float(
                            getattr(
                                self.scope_controller, "focal_mm", focal_length
                            )
                        ),
                    )
                )
        ...
        if scope_data_state == "loading_deep":
            capture_overlay_text = getTraduction(
                "Scope.CapturingPhoto", "Tomando foto..."
            )
        elif scope_data_state == "error_deep":
