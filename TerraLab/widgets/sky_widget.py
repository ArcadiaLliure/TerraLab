...
# ... (Previous code) ...
    ...
    def update_loop(self):
        # Dynamic cadence:
        # - scope / interaction / animation => target 60 FPS
        # - rain => 30 FPS
        # - idle sky => 20 FPS
        target_interval_ms = 50
        try:
            in_scope = bool(getattr(self.canvas, "scope_mode_enabled", lambda: False)())
            interacting = bool(
                getattr(self.canvas, "dragging", False)
                or getattr(self.canvas, "_scope_reticle_drag_active", lambda: False)()
                or (self.anim_timer.isActive() if hasattr(self, "anim_timer") else False)
            )
            if in_scope or interacting:
                target_interval_ms = 16
            elif hasattr(self, "weather") and float(getattr(self.weather, "precip_int", 0.0)) > 0.1:
...
class AstronomicalWidget(CustomWidgetBase):
    ...
    def update_loop(self):
        ...
        if self.use_real_time:
            ...
            # Manual Mode: "Time keeps running forward"
            # Increment manual_hour by elapsed time
            dt_hours = max(0.001, float(getattr(self.timer, "interval", lambda: 50)())) / 1000.0 / 3600.0
