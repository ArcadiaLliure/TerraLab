...
# --- CONFIG ---
DEBUG_CLOUD_DENSITY = (
    1.0  # Multiplicador de quantitat de núvols (1.0 = normal)
)
...
class WeatherPalette:
    """
    Genera i gestiona paletes de clima per a tot l'any.
    24 hores × 365 dies = 8760 registres.
    """

    def __init__(self):
        self.data = []  # Llista de diccionaris amb dades horàries

    def generate_random(
        self,
        clear_nights=0.7,  # 0.0-1.0: més clar = més estrelles
        clear_days=0.6,
        rainy_tendency=0.2,  # 0.0-1.0: probabilitat de pluja
        snowy_tendency=0.1,
    ):  # 0.0-1.0: probabilitat de neu
        """Genera una paleta procedural suau per a tot l'any."""
        self.data = []

        # Generadors de soroll suau (Perlin simplificat)
        def smooth_noise(x, octaves=3):
            """Soroll suau entre 0 i 1."""
            value = 0
            amplitude = 1.0
            frequency = 1.0
            max_value = 0
            ...
            for _ in range(octaves):
                value += amplitude * (
                    math.sin(x * frequency * 0.01) * 0.5 + 0.5
                )
                max_value += amplitude
                amplitude *= 0.5
                frequency *= 2.0
        ...
        for day in range(365):
            ...
            for hour in range(24):
                ...
                cloud_cover = (
                    1.0 - (clear_nights if is_night else clear_days)
                ) * base_noise
                ...
                # LÒGICA FORÇADA: la pluja implica núvols
    ...
    def get_weather(self, day, hour):
        """Obté el clima per a un dia i una hora concrets."""
        day = int(day) % 365
        hour = int(hour) % 24
        idx = day * 24 + hour
...
class Cloud:
    """
    Núvol orgànic dispers (estil fractal).

    Més petit, més irregular i sense "base plana" caricaturesca.
    """
    ...
    def __init__(self, azimuth, altitude, size_deg, puffiness=1.0):
        ...
        scale_factor = 14
        w = int(
            size_deg * scale_factor * 4.0
        )  # Much wider canvas to avoid clipping
    ...
    def move_with_wind(self, dx, dy):
        """
        Mou el núvol en un pla imaginari de "sostre" i recalcula Az/Alt.

        Això permet que els núvols es moguin cap a l'horitzó o cap al zenit
        amb la perspectiva correcta.
        """
        # 1. Projectar (Az, Alt) -> (x, y) en un pla a altura H
        # Fem servir Alt=90 com a centre (distància 0)
        # Distància des del zenit 'r'
        # Com més baix a l'horitzó (Alt->0), més lluny (r->gran)
        # Mapeig simple: r = (90 - Alt) * k
        # Però volem perspectiva tangencial real: r = H / tan(Alt)
        # Evitem divisió per zero per a Alt=0

        eff_alt = max(0.1, self.altitude)
        # Radi projectat des de l'eix Z (zenit)
        # Fem servir una escala arbitrària on l'horitzó visual és a R=1000
        ...
        # Coordenades en el pla del sostre (nord/sud/est/oest)
        ...
        # Azimut
        # atan2(x, y) retorna l'angle des de l'eix Y (nord) si invertim o girem
        # L'atan2(y, x) estàndard parteix de X.
        # Ho ajustem a la nostra convenció anterior
...
class Particle:
    """Partícula simple per a pluja/neu (espai de pantalla)."""
    ...
    def __init__(self, x, y, type="rain"):
        ...
        self.life = 1.0
        self.vy = (
            random.uniform(15, 25) if type == "rain" else random.uniform(2, 5)
        )
        self.vx = (
            random.uniform(-1, 1) if type == "rain" else random.uniform(-2, 2)
        )
    ...
    def draw(self, painter):
        alpha = int(self.life * 150)  # Semitransparent
        if self.type == "rain":
            ...
            painter.drawLine(
                int(self.x),
                int(self.y),
                int(self.x + self.vx * 0.5),
                int(self.y + self.vy * 0.5),
            )
        else:
...
class WeatherSystem:
    def __init__(
        ...
        self._stratus_detail_texture = self._build_stratus_texture(
            160, seed=811
        )
    ...
    def _slot_from_year_day_hour(year, day, hour_value):
        dt = datetime(int(year), 1, 1) + timedelta(
            days=int(day), hours=float(hour_value)
        )
    ...
    def _blend_weather_slots(self, w0, w1, t):
        ...
        out["precipitation_type"] = str(
            nearest.get("precipitation_type", "none")
        )
        return out
    ...
    def _build_stratus_texture(self, size, seed=None):
        ...
        rng = random.Random(
            int(seed) if seed is not None else (size_i * 37 + 11)
        )
        ...
        for y in range(size_i):
            ...
            for x in range(size_i):
                ...
                for freq, phase_x, phase_y, amp in harmonics:
                    ...
                    accum += (
                        0.50 * wave_1 + 0.35 * wave_2 + 0.25 * wave_3
                    ) * amp
                ...
                c = int(202 + 44 * (v**0.78))
                image.setPixelColor(
                    x, y, QColor(c, min(255, c + 4), min(255, c + 12), alpha)
                )
    ...
    def get_runtime_status(self):
        ...
        try:
            requires_user_agent = not bool(
                str(getattr(self.provider, "user_agent", "") or "").strip()
            )
        except Exception:
            requires_user_agent = True
        return {
            "source": str(
                getattr(self, "last_weather_source", "fallback") or "fallback"
            ),
            "reason": str(
                getattr(self, "last_weather_reason", "unknown") or "unknown"
            ),
    ...
    def update_weather(self, day, hour, year=None):
        ...
        self._delta_seconds = max(
            0.01,
            min(
                0.50,
                now_m - float(getattr(self, "_last_update_monotonic", now_m)),
            ),
        )
        ...
        if not isinstance(w, dict):
            ...
            w = self._blend_weather_slots(
                p0, p1, frac_hour
            ) or self.palette.get_weather(d0 % 365, h0)
            self.last_weather_source = "fallback"
        ...
        self._has_layer_cloud_data = all(
            k in w for k in ("cloud_low", "cloud_mid", "cloud_high")
        )
        ...
        self.current_cover_low += (
            cloud_low - self.current_cover_low
        ) * alpha_cloud
        self.current_cover_mid += (
            cloud_mid - self.current_cover_mid
        ) * alpha_cloud
        self.current_cover_high += (
            cloud_high - self.current_cover_high
        ) * alpha_cloud
        self.fog_cover += (
            float(w.get("fog_cover", 0.0)) - self.fog_cover
        ) * alpha_haze
        self.humidity += (
            float(w.get("humidity", 0.0)) - self.humidity
        ) * alpha_haze
        self.precip_rate_mm_h += (
            float(w.get("precip_rate_mm_h", 0.0)) - self.precip_rate_mm_h
        ) * alpha_precip
        self.precip_int += (
            float(w.get("precipitation_intensity", 0.0)) - self.precip_int
        ) * alpha_precip
    ...
    def _manage_clouds(self):
        ...
        target_density = int(
            (14.0 + 30.0 * (coverage**1.15)) * DEBUG_CLOUD_DENSITY
        )
        ...
        for layer in ("low", "mid", "high"):
            ...
            spawn_attempts = (
                ...
                + int(deficit > 8)
                + int(deficit > 18)
                + (2 if self._slot_changed_recent else 0)
            )
        ...
        if len(self.clouds) > sum(
            layer_targets.values()
        ) and random.random() < (0.28 if self._slot_changed_recent else 0.18):
            removals = (
                1
                + int(len(self.clouds) > sum(layer_targets.values()) + 10)
                + (2 if self._slot_changed_recent else 0)
            )
            for _ in range(removals):
                for layer in ("high", "mid", "low"):
                    if layer_counts.get(layer, 0) > layer_targets.get(
                        layer, 0
                    ):
        ...
        delta_s = max(
            0.01, min(0.25, float(getattr(self, "_delta_seconds", 0.05)))
        )
    ...
    def _manage_precip(self):
        ...
        base_count = self.precip_rate_mm_h * (
            14.0 if self.precip_type == "rain" else 10.0
        )
        intensity_count = self.precip_int * (
            20.0 if self.precip_type == "rain" else 14.0
        )
        count = int(max(0.0, base_count + intensity_count))

        for _ in range(count):
            self.particles.append(
                Particle(random.uniform(0, self.width), -10, self.precip_type)
            )
    ...
    def update_thunder(self):
        if (
            self.thunder_prob > 0
            and random.random() < self.thunder_prob * 0.01
        ):
            self.flash_val = 1.0
        if self.flash_val > 0:
            self.flash_val *= 0.85
    ...
    def draw(
        ...
        # 2. PARTIAL ECLIPSE -> Transition
            ...
            if base_tint is None:
                ...
                alpha_val = int(
                    min(255, darkness * 300)
                )  # Ramp faster to opaque
                ...
                r = int(
                    base_tint.red() * (1.0 - progress) + target_r * progress
                )
                g = int(
                    base_tint.green() * (1.0 - progress) + target_g * progress
                )
                b = int(
                    base_tint.blue() * (1.0 - progress) + target_b * progress
                )
                a = int(
                    base_tint.alpha() * (1.0 - progress) + target_a * progress
                )
        ...
        if eff_sun_alt >= -2.0:
            ...
            alpha_top = max(
                0, min(255, int((1.0 - t_top) * 210.0 + 24.0 * precip_dark))
            )
            alpha_mid = max(
                0, min(255, int((1.0 - t_mid) * 225.0 + 28.0 * precip_dark))
            )
            alpha_bottom = max(
                0, min(255, int((1.0 - t_bottom) * 242.0 + 36.0 * precip_dark))
            )
            ...
            grad_cov.setColorAt(
                0.0, QColor(gray + 6, gray + 8, min(255, gray + 14), alpha_top)
            )
            grad_cov.setColorAt(
                0.55,
                QColor(gray + 2, gray + 4, min(255, gray + 10), alpha_mid),
            )
            grad_cov.setColorAt(
                1.0,
                QColor(gray - 8, gray - 6, min(255, gray + 2), alpha_bottom),
            )
            ...
            if coverage > 0.45:
                ...
                painter.fillRect(
                    ...
                    self.width,
                    self.height,
                    QColor(gray - 5, gray - 3, min(255, gray + 4), over_alpha),
                )
            ...
            if self._stratus_texture is not None:
                ...
                painter.drawTiledPixmap(
                    QRectF(-ox, -oy, self.width + tw, self.height + th),
                    self._stratus_texture,
                )
            if self._stratus_detail_texture is not None:
                ...
                detail_opacity = max(
                    0.06, min(0.48, 0.10 + (1.0 - t_bottom) * 0.35)
                )
                painter.setOpacity(detail_opacity * 0.95)
                painter.drawTiledPixmap(
                    QRectF(-dox, -doy, self.width + dw, self.height + dh),
                    self._stratus_detail_texture,
                )
            painter.restore()

        # Ordena els núvols per altitud per pintar primer els més propers a
        # l'horitzó (fons)
        # i després els del zenit (primer terme)?
        # En una volta celeste, el més llunyà és l'horitzó.
        # Pintem d'horitzó (0) a zenit (90)?
        # Sí, normalment en ordre de pintat.
        ...
        for c in sorted_clouds:
            if project_fn is not None:
                ...
                if (
                    screen_x < -320
                    or screen_x > (self.width + 320)
                    or screen_y < -220
                    or screen_y > (self.height + 260)
                ):
                    continue
            else:
                ...
                if not (
                    abs(diff_az) < (current_fov / 2 + 30)
                    and abs(diff_alt) < (current_fov / 2 + 30)
                ):
                    continue
            ...
            # Els núvols a l'horitzó (alt=0) es veuen més petits i aixafats.
            # Els núvols al zenit (alt=90) es veuen grans (són més a prop).
            # Factor d'escala per altitud:
            # Horitzó: 0.4x, zenit: 1.5x
            ...
            # Escala final combinant FOV (zoom) i perspectiva
            ...
            if base_tint is None:
                ...
                day_cloud_opacity = 0.58 + 0.34 * max(
                    0.0, min(1.0, self.current_cover)
                )
            ...
            # Aplasta els núvols a l'horitzó per efecte 3D
            if c.altitude < 20:
                flatten = 0.5 + (c.altitude / 20.0) * 0.5
                painter.scale(1.0, flatten)
        ...
        if self.fog_cover > 0.01 or self.humidity > 0.75:
            fog_strength = min(
                1.0,
                self.fog_cover * 1.15 + max(0.0, self.humidity - 0.75) * 1.6,
            )
        ...
        if self.flash_val > 0.01:
            painter.fillRect(
                ...
                self.width,
                self.height,
                QColor(255, 255, 255, int(self.flash_val * 128)),
            )
