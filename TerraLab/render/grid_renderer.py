...
def draw_celestial_grid_impl(canvas, painter, hour):
    ...
    lst = (100.0 + day_of_year * 0.9856 + float(hour) * 15.0 + canvas.parent_widget.longitude) % 360
    ...
    for i in range(steps + 1):
        ...
        cos_az = (-sin_alt * math.sin(lat_rad)) / (math.cos(math.radians(alt)) * math.cos(lat_rad) + 1e-10)
