...
def build_star_scene_state(canvas, hour, sun_alt, sun_az, mag_limit, eff_lat, day_of_year):
    ...
    extras = {
        ...
        "stars_enabled": bool(canvas._parent_checkbox_checked("chk_enable_sky", default=True)),
        ...
        "scope_sync_index_build_max_rows": int(
            max(50_000, int(get_config_value("performance.scope_sync_index_build_max_rows", 750_000)))
        ),
        "scope_allow_sync_index_build": bool(get_config_value("performance.scope_allow_sync_index_build", False)),
        "scope_pending_prefilter_cap_interaction": int(
            max(5_000, int(get_config_value("performance.scope_pending_prefilter_cap_interaction", 40_000)))
        ),
        "scope_pending_prefilter_cap_static": int(
            max(5_000, int(get_config_value("performance.scope_pending_prefilter_cap_static", 70_000)))
        ),
        "scope_pre_altaz_max_candidates_pending": int(
            max(5_000, int(get_config_value("performance.scope_pre_altaz_max_candidates_pending", 60_000)))
        ),
        "scope_pre_altaz_max_candidates_interaction": int(
            max(10_000, int(get_config_value("performance.scope_pre_altaz_max_candidates_interaction", 120_000)))
        ),
        "scope_pre_altaz_max_candidates_static": int(
            max(10_000, int(get_config_value("performance.scope_pre_altaz_max_candidates_static", 220_000)))
        ),
    }
    ...
    scope_focal_mm = float(getattr(getattr(canvas, "scope_controller", None), "focal_mm", 250.0))
    ...
    if vm_state is not None:
        try:
            ...
            exposure_factor = max(0.0, min(1.0, math.log2(exposure_seconds) / 5.0))
    ...
    if scope_enabled and hasattr(canvas, "scope_controller"):
        ...
        if center is None:
            center = (float(canvas.elevation_angle), float(canvas.azimuth_offset))
        if center is not None:
            try:
                center_alt = float(center[0])
                center_az = float(center[1]) % 360.0
                center_ra_dec = canvas._altaz_to_ra_dec(center_alt, center_az, float(hour), int(day_of_year))
    ...
    return SceneState(
        ...
        naked_eye_cap=float(
            min(float(getattr(pw, "magnitude_limit", STAR_CATALOG_NAKED_EYE_MAX_MAG)), STAR_CATALOG_NAKED_EYE_MAX_MAG)
        ),
        scope_mask_fn=canvas._scope_contains_alt_az_mask,
        horizon_profile=getattr(getattr(canvas, "horizon_overlay", None), "profile", None),
        extras=extras,
    )
