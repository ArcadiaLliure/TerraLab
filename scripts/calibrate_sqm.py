...
def main():
    parser = argparse.ArgumentParser(
        description="Calibrate DVNL to SQM model."
    )
    parser.add_argument(
        "csv", help="CSV with columns: lat, lon, agg_dvnl, elevation_m, sqm"
    )
    ...
    model.fit(
        df["agg_dvnl"].values, df["elevation_m"].values, df["sqm"].values
    )
