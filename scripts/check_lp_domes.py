...
try:
    ...
    print(
        "Error: Asegúrate de instalar las dependencias con 'pip install -r requirements.txt'"
    )
    sys.exit(1)
...
def main():
    ...
    tiff_path = os.path.join(
        base_path, "TerraLab", "data", "light_pollution", "C_DVNL 2022.tif"
    )
    ...
    for p in peaks[:8]:
        print(
            f"Azimut: {p[0]:3d}° | Distancia: {p[1]/1000:4.1f}km | Radiancia: {p[2]:.2f}"
        )
