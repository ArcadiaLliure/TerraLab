...
def main():
    parser = argparse.ArgumentParser(
        description="Convert SQM map to m_lim map."
    )
    parser.add_argument("sqm_in", help="Input SQM raster (.tif)")
    parser.add_argument("mlim_out", help="Output m_lim raster (.tif)")
    parser.add_argument(
        "--alt", type=float, default=90.0, help="Altitude angle (degrees)"
    )
    parser.add_argument(
        "--k", type=float, default=0.25, help="Extinction coefficient"
    )
    ...
    v_mlim = np.vectorize(
        lambda s: (
            calculate_mlim_from_sqm(s, args.alt, args.k)
            if not np.isnan(s)
            else np.nan
        )
    )
