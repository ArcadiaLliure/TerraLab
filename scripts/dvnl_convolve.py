...
def main():
    parser = argparse.ArgumentParser(
        description="Convolve DVNL raster with a dispersion kernel."
    )
    parser.add_argument(
        "input", help="Preprocessed DVNL .tif (float32, nan for nodata)"
    )
    ...
    parser.add_argument(
        "--sigma", type=float, default=30.0, help="Sigma for Gaussian (km)"
    )
    parser.add_argument(
        "--rmax", type=float, default=200.0, help="Cutoff radius (km)"
    )
    parser.add_argument(
        "--res", type=float, default=1.0, help="Resolution (km/pixel)"
    )
        ...
        with rasterio.open(args.output, "w", **meta) as dst:
            ...
            for i in tqdm(range(0, src.height, tile_size)):
                for j in range(0, src.width, tile_size):
                    ...
                    dst.write(
                        cropped.astype(np.float32), 1, window=write_window
                    )
